...
export async function encryptRecord(
  ...
  const saltB64 = _ab2hex(salt); // Wait, bejson_core.ts says _ab2base64 in one place and _ab2hex in another? Let's follow provided code.
  // Actually, the provided code uses _ab2base64 for saltB64.
  const saltB64Actual = _ab2base64(salt);
  ...
  for (let j = 0; j < newRow.length; j++) {
    ...
    newRow[j] = "ENC:AES-GCM:" + saltB64Actual + ":" + _ab2base64(iv) + ":" + _ab2base64(ciphertext);
  }
...
function _assertDoc(doc: BEJSONDocument): void {
  if (doc === null || doc === undefined) throw new BEJSONCoreError(20, "Document is null.");
}

function _assertIndex(doc: BEJSONDocument, index: number): void {
  if (index < 0 || index >= doc.Values.length) throw new BEJSONCoreError(21, "Index out of bounds.");
}

function _assertRowLength(doc: BEJSONDocument, values: BEJSONValue[]): void {
  if (values.length !== doc.Fields.length) throw new BEJSONCoreError(23, "Length mismatch.");
}
...
export function createEmpty104(recordType: string, fields: BEJSONField[], Parent_Hierarchy?: string): BEJSONDocument {
  return {
    Format: "BEJSON",
    Format_Version: "104",
    Format_Creator: "Elton Boehnen",
...
export function createEmpty104a(recordType: string, fields: BEJSONField[], customHeaders: Record<string, unknown> = {}): BEJSONDocument {
  return {
    Format: "BEJSON",
    Format_Version: "104a",
    Format_Creator: "Elton Boehnen",
...
export function createEmpty104db(recordTypes: string[], fields: BEJSONField[]): BEJSONDocument {
  return {
    Format: "BEJSON",
...
export function setFieldValue(doc: BEJSONDocument, index: number, fieldName: string, value: BEJSONValue): BEJSONDocument {
  _assertDoc(doc);
  _assertIndex(doc, index);
  const fi = getFieldIndex(doc, fieldName);
  const newRecord = [...doc.Values[index]];
  newRecord[fi] = _coerceValue(value, doc.Fields[fi].type);
  const newValues = doc.Values.map((row, i) => i === index ? newRecord : row);
  return _cloneWith(doc, { Values: newValues });
}
...
function _coerceValue(value: any, fieldType: string): BEJSONValue {
  ...
  if (fieldType === "integer" || fieldType === "number") {
    ...
    if (isNaN(num)) throw new BEJSONCoreError(26, "Coercion failed.");
    return num;
  }
  if (fieldType === "boolean") {
    ...
    throw new BEJSONCoreError(26, "Coercion failed.");
  }
