...
class SwitchRenderer {
    ...
    drawChunkedLayer(chunkManager, loadedImages, tileSize, assets) {
        if (!this.ctx || !chunkManager || !loadedImages) return;
        ...
        chunkManager.activeChunks.forEach(doc => {
            ...
            for (const tile of doc.Values) {
                ...
                const object = tile[5];
                const spriteId = terrain || object;
                if (!spriteId) continue;

                if (loadedImages[spriteId] && loadedImages[spriteId].complete && loadedImages[spriteId].naturalWidth > 0) {
                    this.ctx.drawImage(
                        loadedImages[spriteId],
                        tx - this.camera.x,
                        ty - this.camera.y,
                        tileSize, tileSize
                    );
                } else if (assets && assets[spriteId]) {
                    this.ctx.fillStyle = assets[spriteId].fallback_color || '#000000';
                    this.ctx.fillRect(tx - this.camera.x, ty - this.camera.y, tileSize, tileSize);
                }
            }
        });
    }
